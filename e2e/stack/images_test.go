...
func TestResolveImages_Namespaces(t *testing.T) {
	...
	for svc, ref := range relocated {
		if want := "ghcr.io/baseproof/tooling/" + svc + ":0.0.22"; ref != want {
			t.Errorf("%s image = %q, want %q (relocated fleet, 0.0.22 release)", svc, ref, want)
		}
...
// ledger variant under the new namespace/tag (ghcr's 0.0.22-upstream tag).
func TestResolveImages_TesseraUpstream(t *testing.T) {
	clearImageEnv(t)
	t.Setenv("E2E_TESSERA", "upstream")
	im := ResolveImages()
	if want := "ghcr.io/baseproof/tooling/ledger:0.0.22-upstream"; im.Ledger != want {
		t.Errorf("upstream ledger = %q, want %q", im.Ledger, want)
	}
	// Only the ledger has an upstream variant; witness/auditor are unaffected.
	if want := "ghcr.io/baseproof/tooling/witness:0.0.22"; im.Witness != want {
