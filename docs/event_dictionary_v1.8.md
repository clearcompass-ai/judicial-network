...
The judicial-network records every substantive event on Baseproof, an append-only, cryptographically signed log substrate — conceptually similar to Certificate Transparency logs. Entries are permanent and order-preserving; integrity is verifiable by anyone with access to the log.
