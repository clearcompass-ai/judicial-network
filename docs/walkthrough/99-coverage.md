...
3. The numerator in "12 of 17 payload schemas" should match
   `grep -rhoE 'type [A-Za-z]+Payload struct' schemas/ | sort -u | wc -l`
   after each schema add (modulo the few non-payload schemas in that
   directory).
...
- **Twelve of seventeen** payload schemas exercised, with the
  remaining five listed as one-spec extensions.
