...
DESCRIPTION:
    ...
    the witness BLS key and the operator endpoints.
...
type Config struct {
	...
	// emitted cosignatures so the operator can route them to the
	...
	// have a matching entry in Operators.
	LogDIDs []string `json:"log_dids"`

	// Operators maps log DID → operator base URL. The daemon polls
...
func (c Config) Validate() error {
	...
	if len(c.Operators) == 0 {
		return fmt.Errorf("witness: operators map cannot be empty")
	}
	for _, did := range c.LogDIDs {
		if _, ok := c.Operators[did]; !ok {
			return fmt.Errorf("witness: log_did %q has no operator endpoint", did)
		}
	}
	return nil
}
