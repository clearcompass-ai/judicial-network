...
Polls the ledger for new entries on the registered logs
...
```
       ...
       │     GET /readyz    ledger + Postgres reachable   │
...
| Field | Purpose |
|---|---|
| `ledger_url`              | Ledger HTTP endpoint to poll. Required. |
...
| `GET /readyz`  | Readiness. 200 only when Postgres + ledger are both reachable; 503 otherwise. k8s removes from service when readyz drops. |
...
**Ledger availability.** Same model — readyz drops when the
ledger is unhealthy. The  reliability primitives
...
| `TestRun_RejectsEmptyDatabaseURL` / `_LedgerURL` | config validation |
...
| `TestProbes_Readyz_HappyPath` / `_DBDown_503` / `_LedgerDown_503` | readiness gates traffic correctly |
