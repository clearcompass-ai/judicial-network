...
DESCRIPTION:
    ...
    Helpers (unsignedEntryFor, cosignedDisplay, fakeOperator,
...
func TestSignAndSubmitCosigned_RejectsEmptyCosigners(t *testing.T) {
	...
	bc := newBuildContext(t, sp, &fakeOperator{})
...
func TestSignAndSubmitCosigned_RejectsNilDeps(t *testing.T) {
	...
	bc := newBuildContext(t, identity.NewStubProvider(), &fakeOperator{})
...
func TestSignAndSubmitCosigned_PrimaryRejection(t *testing.T) {
	...
	if len(op.captured) != 0 {
		t.Errorf("operator must not see entries when primary rejects")
	}
}
...
func TestSignAndSubmitCosigned_CosignerRejection(t *testing.T) {
	...
	if len(op.captured) != 0 {
		t.Errorf("operator must not see entries when cosigner rejects")
	}
}
